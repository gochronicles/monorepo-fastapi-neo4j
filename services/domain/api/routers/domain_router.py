...
def createDomain(create_domain_request: Domain):
    ...
    Args:
        create_domain_request (Domain): [name: str, total_pages: int, document_count: int,
       classified: bool]
...
def updateDomain(update_domain_request: DomainUpdate):
    ...
    Args:
        update_domain_request (Domain): [name: str, total_pages: int, document_count: int,
       classified: bool]
...
def getAllDomaines():
    """Get all the domains present in the database.
    ...
    Returns:
        list: list of all the domains record.
    """
    logging.debug("Router: /domain/all")
    logging.debug(f"Router getAllDomaines")

    try:
        return DomainController.get_all_domains()
    except DomainException as response_error:
        raise HTTPException(response_error.status_code, response_error.error_detail)
...
def deleteAllDomaines():
    ...
    try:
        return DomainController.delete_all_domains()
    except DomainException as response_error:
        raise HTTPException(response_error.status_code, response_error.error_detail)
